...
另有一个**人工回归工具**，故意不叫 `verify:`（不进 gate、不进 CI）：

```sh
npm run card:live -- <会话日志路径>     # 例如 ~/.dsh/sessions/<项目>/<会话>/session.v3.jsonl.zstd
```

它读一份真实会话日志（多帧 zstd，按帧解压），把里面**持久化的卡片**逐条用客户端模型读回并量字节——这是唯一能看到预算在真实数据上如何表现的场合（中文三字节、超宽单元格、DECIMAL/BIGINT 字符串）。它只断言「每个工具**最新**一张卡在预算内」（新写的才代表当前代码），更早的卡片是历史版本写的，只报告不判失败。会话日志是用户数据，所以路径必须显式传。
